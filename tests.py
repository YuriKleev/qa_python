...
class TestBooksCollector:
    ...
    def test_add_new_book_add_book_twice_second_book_not_added (self, collector):
        ...
        collector.add_new_book('Гиперион')
        assert collector.get_books_rating.count('Гиперион') == 1

    def test_add_new_book_added_book_without_genre (self, collector):
        collector.add_new_book('Гиперион')
        assert collector.books_genre('Гиперион') == ''
    ...
    @pytest.mark.parametrize('genre_name', ['Фантастика','Комедии'])
    def test_get_books_with_specific_genre_existent_genre (self, collector):
        collector.add_new_book('Гиперион')
        collector.set_book_genre('Гиперион','Фантастика')
        assert collector.get_books_with_specific_genre(self, 'Фантастика') == ['Гиперион']
    ...
    def test_get_books_with_specific_genre_nonexistent_genre (self, collector, genre_name):
        collector.add_new_book('Гиперион')
        collector.set_book_genre('Гиперион','Фантастика')
        assert collector.get_books_with_specific_genre(self, genre_name) == []
    ...
    def test_delete_book_from_favorites_delete_existent_book_book_deleted (self, collector, name):
        collector.add_new_book('Гиперион')
        collector.add_book_in_favorites('Гиперион')
        collector.delete_book_from_favorites('Гиперион')
        assert collector.get_list_of_favorites_books().count('Гиперион') == 0

    def test_delete_book_from_favorites_delete_nonexistent_book (self, collector, name):
